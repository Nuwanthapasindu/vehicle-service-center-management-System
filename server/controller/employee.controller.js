...
const { validatedCreateEmployee, validatedUpdateEmployee } = require("../validation/employee.validation");
...
exports.getEmployees = async (query) => {
    try {
        ...
        const employees = await Employee.find(filters).populate({ path: "user", select: ["-__v", "-isDeleted", "-deletedAt"] }).select(["-__v", "-isDeleted", "-deletedAt"]);
        return employees;
...
exports.updateEmployee = async (id, payload) => {
    ...
    try {
        const { name, mobile, address, userName, password, dob, nic, skills, gender, isAvailable } = value;
        const employee = await Employee.findById(id);
        if (!employee) throw new AppError("Employee not found", 404);

        // CHECK MOBILE NUMBER ALREADY EXIST
        const existingUser = await User.exists({ mobile: mobile, _id: { $ne: employee.user } });
        if (existingUser) throw new AppError("Mobile number already exist", 400);

        // CHECK NIC ALREADY EXIST
        const existingNic = await Employee.exists({ nic: nic, _id: { $ne: employee._id } });
        if (existingNic) throw new AppError("NIC already exist", 400);
        ...
        await Employee.findByIdAndUpdate(id, { dob, nic, skills, gender, isAvailable });
