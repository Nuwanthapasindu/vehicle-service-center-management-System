...
exports.createTeam = async (payload) => {
    ...
    try {
        ...
        const existingEmployees = await Employee.find({ _id: { $in: employees }, isDeleted: false });
        if (existingEmployees.length !== employees.length) throw new AppError("Employee not found", 400);
...
exports.getAllTeams = async () => {
    try {
        const teams = await Team.find({ isDeleted: false })
            .populate({ path: "employees", select: ["-isDeleted", "-deletedAt", "-__v"] })
...
exports.getTeamById = async (id) => {
    try {// CHECK ID IS A VALID OBJECT ID
        if (!mongoose.Types.ObjectId.isValid(id)) throw new AppError("Invalid Team ID", 400);

        const team = await Team.findById(id).populate({ path: "employees", select: ["-isDeleted", "-deletedAt", "-__v"] }).lean();
...
exports.updateTeam = async (id, payload) => {
    ...
    if (error) throw new AppError(error.details[0].message, 400);
    try {
        const existingTeam = await Team.exists({ name: value.name, isDeleted: false, _id: { $ne: id } });
        if (existingTeam) throw new AppError("Team name already exist", 400);
        if (Array.isArray(value.employees) && value.employees.length > 0) {
            for (const id of value.employees) {
                if (!mongoose.Types.ObjectId.isValid(id)) throw new AppError("Invalid employee ID", 400);
            }
            const existingEmployees = await Employee.find({ _id: { $in: value.employees }, isDeleted: false });
            if (existingEmployees.length !== value.employees.length) throw new AppError("Employee not found", 400);
        }
