...
exports.createJobCard = async (payload) => {
    ...
    const existingCard = await JobCard.findOne({ booking: value.booking, isDeleted: false });
...
exports.getMyTasks = async (user) => {
    if (!user || !user.nic) throw new AppError("Employee identity not found", 401);

    const employeeRecord = await Employee.findOne({ nic: user.nic, isDeleted: false });
    if (!employeeRecord) throw new AppError("Employee record not found", 404);

    const myTeams = await Team.find({ employees: employeeRecord._id, isDeleted: false }).select('_id');
...
exports.getAllBookings = async () => {
    ...
    const existingJobCardBookingIds = await JobCard.find({ isDeleted: false }).distinct('booking');
